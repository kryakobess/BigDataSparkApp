...
class ExperimentConfig:
    ...
    output_prefix: str
    spark_builder: Callable[[SparkSession.Builder, ExperimentArgs], SparkSession.Builder]
    transform_after_prepare: Optional[Callable[[DataFrame, ExperimentArgs], DataFrame]] = None
    extra_spark_config: Optional[Callable[[ExperimentArgs], Dict[str, Any]]] = None
...
def build_spark_session(args: ExperimentArgs, config: ExperimentConfig) -> SparkSession:
    ...
    builder = config.spark_builder(builder, args)
...
def resolve_location_column(df: DataFrame) -> str:
    if "PULocationID" in df.columns:
        return "PULocationID"
    if ("pickup_longitude" in df.columns and "pickup_latitude" in df.columns) or "pickup_zone_approx" in df.columns:
...
def write_experiment_log(
    ...
    output_path = os.path.join(args.output_dir, f"{config.output_prefix}_{timestamp}.json")
    ...
    if config.extra_spark_config is not None:
        spark_config.update(config.extra_spark_config(args))
...
def run_experiment(args: ExperimentArgs, config: ExperimentConfig) -> None:
    ...
    spark = build_spark_session(args, config)

    try:
        step_start = log_step_start(logger, "read_dataset")
        ...
        log_step_end(logger, "read_dataset", step_start, step_metrics)

        step_start = log_step_start(logger, "prepare_dataset")
        ...
        log_step_end(logger, "prepare_dataset", step_start, step_metrics)

        working_df = prepared_df
        if config.transform_after_prepare is not None:
            step_start = log_step_start(logger, "optimize_dataset")
            working_df = config.transform_after_prepare(prepared_df, args)
            logger.info(
                "Dataset transformed | partitions=%d",
                working_df.rdd.getNumPartitions(),
            )
            log_step_end(logger, "optimize_dataset", step_start, step_metrics)

        step_start = log_step_start(logger, "collect_metrics")
        result_metrics = collect_metrics(working_df)
        logger.info(
            "Metrics collected | total_rows_after_cleanup=%d",
            result_metrics["total_rows_after_cleanup"],
        )
        log_step_end(logger, "collect_metrics", step_start, step_metrics)
        ...
        step_start = log_step_start(logger, "write_experiment_log")
        ...
        log_step_end(logger, "write_experiment_log", step_start, step_metrics)

        logger.info(
            "%s | total_duration_sec=%.3f | peak_driver_ram_mb=%.2f",
            config.finished_message,
            total_duration_sec,
            peak_driver_ram_mb,
        )
    finally:
        if config.transform_after_prepare is not None and "working_df" in locals():
            working_df.unpersist(blocking=False)
        spark.stop()
